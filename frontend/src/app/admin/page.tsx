...
export default async function AdminDashboard() {
  const productsCount = DUMMY_PRODUCTS.length;
  const ordersCount = 2; // dummy
  
  const allOrders = [
    { id: "ord_1", customer: "johndoe@email.com", status: "Processing", total: 120 },
    { id: "ord_2", customer: "alexsmith@email.com", status: "Shipped", total: 180 },
  ];

  const totalRevenue = 300; // dummy
  const uniqueCustomersCount = 2; // dummy

  const stats = [
    { title: "Total Revenue", value: `₹${(totalRevenue || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, trend: "+20.1%", icon: DollarSign },
    ...
    { title: "Customers", value: uniqueCustomersCount.toString(), trend: "+2%", icon: Users },
  ];
